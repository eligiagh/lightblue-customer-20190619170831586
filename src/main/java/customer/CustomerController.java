...
private Database cloudant;
	@private CloudantPropertiesBean
cloudantProperties;
	...
	.password(cloudantProperties.getPassword())
						.build();
				cloudant=
	cloudantClient.database(cloudantProperties.getDatabase(), true);			
...
public class CustomerController {
    ...
    ResponseEntity<?> create(@RequestHeader Map<String, String> headers, @RequestBody Customer payload) {
    	try {
    		if (payload.getCustomerId() != null && cloudant.contains(payload.getCustomerId())) {
    			return
    ResponseEntity.badRequest().body("Id " + payload.getCustomerId() + " already exists");
    		}
    		final List<Customer> customers = cloudant.findByIndex(
    																	...
    																	if (!customers.isEmpty()) {
    																		return
    												ResponseEntity.badRequest().body ("Customer with name " + payload.getUSername() + " already exists");
    													}
    													final Response resp =
    ...
    ResponseEntity<?> getById(@RequestHeader Map<String, String> headers, @PathVariable String id) {
        try {
        	final Customer cust =
        cloudant.find(Customer.class, id);
    ...
    getCustomers (@RequestHeader Map<String, String> hdrs) {
    	try {
